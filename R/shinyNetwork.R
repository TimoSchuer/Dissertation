shinyNetwork <- function(exb){
  shiny::shinyApp(
    ...
    server = function(input, output){
     ...
     shiny::eventReactive(input$saveToVar,{NetworkPlot()[[1]] %>% dplyr::select(c("IPId","File", "Name", "Text", input$vars, "group"))%>%  assign("ShinyAnalyse",., envir = .GlobalEnv) %>% as.data.frame()})
